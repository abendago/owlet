...
class Query
{
	...
	*/
	private static function serializeArray($array)
	{
		...
		foreach ($array AS $key => $val) {
			$addquotes = (is_string($val) && !is_numeric($val) && !self::isQuery($val) && !strstr($val, "()"));
			$wrapper = $addquotes ? "%s = '%s'" : "%s = %s";
			if ($val === null) $val = "NULL";
			$str .= sprintf($wrapper, $key, $val) . self::sql_delimiter;
		}
		return rtrim($str, self::sql_delimiter);
	}
	...
	*/
	public static function update($query_table, $update_values, $conditional_values)
	{
		$sql_set = self::serializeArray($update_values);
		$sql_where = self::serializeArray($conditional_values);
		return self::execute("UPDATE {$query_table} SET {$sql_set} WHERE {$sql_where}");
	}
	...
	*/
	...
	{
		$sql_where = self::serializeArray($conditional_values);
		return self::execute("DELETE FROM {$query_table} WHERE {$sql_where}");
	}
