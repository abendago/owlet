...
/*
[NOTES]
A database class that provides methods to work with mysql,
...
class db {
  ...
  */
  function db($cnn_id=0) {
    ...
    $this->cnn_id = $cnn_id;
    $this->dir = realpath("./")."/_cache/"; //Default to current dir.
    $this->ttl = 0;
    $this->data_ts = 0;
  }
  ...
  function connect($newDBInfo) {
    ...
    switch($this->cnn_id) {
      ...
      */
      case 0:
        $this->db_type = "mysql";
        $this->server = $newDBInfo[host];
        $user 		    = $newDBInfo[username];
        $pwd 		      = $newDBInfo[password];
        $this->db     = $newDBInfo[dbname];
        break;
    }
  ...
  */
  function fetch($sql="",$ttl=0,$name="", $byID="") {
    ...
    $data = "";
    if ($this->ttl == "0") {
      return $data = $this->getFromDB($byID);
    } else {
      ...
      if ($this->ttl == "daily") {
        if (date('Y-m-d',$this->data_ts) != date('Y-m-d',time())) {
          ...
          if (!$data = $this->getFromDB($byID)) { return false; }
  ...
  */
    ...
    switch($this->db_type) {
      case "mysql":
        ...
        while ($row = mysql_fetch_array($res, MYSQL_ASSOC)) {
          if ($byID)
			{
			  $data[array_shift(array_values($row))] = $row;
			} else {
			  $data[] = $row;
			}
        }
  ...
  function eraseCacheFile($strFilename)
	{
	  if ($strFilename){
//		mail("nathan@abendago.com", "delete", $this->dir."db_".$this->name);
		unlink($this->dir."db_".$strFilename);
	  }
	}
